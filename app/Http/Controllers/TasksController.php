...
elseif ($task->created_by !== $userid) return response()->json([
 'error' => 'Unauthorized'
],401);
...
else if ($task->status === 'completed' && $request->input('status') !== 'completed') return response()->json([
 'error' => 'Task already marked as completed'
],401);
elseif ($task->assigned_to !== $userid ) return response()->json([
 'error' => 'Unauthorized'
],401);
else return response()->json([
   'error' => 'Task already deleted'
...
public function sendstats(Request $request){
  ...
  if($condition === "last year"){
    $jobj = Helper::dashboardhelper($year-1,12,$userid,$total);
  }
  else if($condition === "this year"){
   $jobj = Helper::dashboardhelper($year,$mon,$userid,$total);
 }
 else{
  $obj1 = Helper::dashboardhelper($year-1,12,$userid,$total);
  $obj2 = Helper::dashboardhelper($year-2,12,$userid,$total);
